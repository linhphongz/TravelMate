...
def main():
    st.set_page_config(
        page_title="TravelMate 🧳✈️🌍",
        page_icon="🧳",
        layout="centered",
    )

    st.title("TravelMate Chatbot 🧳✈️🌍")
    ...
    if st.session_state.send_input:
        if Image_file and st.session_state.flag_img_file == False:
            ...
            with chat_container:
                with st.chat_message("human"):
                    st.markdown(st.session_state.user_question)
                llm_response = st.session_state.chat.handle_user_question(question=st.session_state.user_question).text
                chat_history.add_user_message(st.session_state.user_question)
